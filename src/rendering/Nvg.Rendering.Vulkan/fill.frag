...
float scissorMask(vec2 p) {
    vec2 sc = (abs((data[fid].scissorMat * vec4(p, 1.0, 1.0)).xy) - data[fid].scissorExt);
...
void main(void) {
    ...
    if (data[fid].type == 0) { // Gradient
        ...
        vec2 pt = (data[fid].paintMat * vec4(fpos, 1.0, 1.0)).xy;
        ...
        vec2 pt = (data[fid].paintMat * vec4(fpos, 1.0, 1.0)).xy / data[fid].extent;
